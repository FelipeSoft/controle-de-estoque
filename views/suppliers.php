...
require_once(dirname(__FILE__) . "../../database/repository/SupplierRepository.php");
require_once(dirname(__FILE__) . "../../database/dao/SupplierDataAccessObjectMySQL.php");
require("../config/config.php");

$dao = new SupplierDataAccessObjectMySQL($connection);
$repository = new SupplierRepository($dao);
$suppliers = $repository->getAllSuppliers();

$title = "Fornecedores - Controle de Estoque";
$session = "Fornecedores";
$session_text = "Cadastre os seus fornecedores de forma facilitada.";

$id = filter_input(INPUT_GET, "supplier_id", FILTER_SANITIZE_STRING);
...
<main class="w-screen">
    ...
    <section class="mx-auto container my-10">
        ...
        <?php
        if (isset($_SESSION['flash_supplier'])) {
            echo '<div class="w-full h-max bg-red-200 mt-8 p-4 text-red-700 border-2 border-red-300 rounded-md">' . $_SESSION['flash_supplier'] . '</div>';
            unset($_SESSION['flash_supplier']);
        }
        ?>
        <div>
            <h1 class="mt-8 text-blue-500 font-bold text-2xl mb-2">Cadastro de Fornecedores</h1>
            <form action="<?= $id ? "../app/actions/edit_supplier.php" : "../app/actions/new_supplier.php" ?>"
                method="post" class="mb-16">
                <div class="grid grid-cols-3 gap-6">
                    <input type="text" value="<?= $id ?>" name="id" hidden>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Nome
                    <input value="<?= $name ?? "" ?>" name="supplier_name" type="text" placeholder="ex: Apple Computer"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Email
                    <input value="<?= $email ?? "" ?>" name="supplier_email" type="email"
                            placeholder="ex: suporte@gmail.com"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Telefone de Contato
                    <input value="<?= $telephone ?? "" ?>" name="supplier_telephone" type="text"
                            placeholder="ex: 19999179999"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                </div>
                    ...
                    <tbody>
                        <?php foreach ($suppliers as $p): ?>
                            <tr class="border-2 even:bg-gray-200">
                                ...
                                <td class="py-2 px-4 text-sm">
                                    <?php
                                    $from_database_datetime = $p->updated_at;
                                ...
                                <td class="p-4 flex items-center justify-center gap-2">
                                    <a href="<?= BASE_URL ?>/views/suppliers.php?supplier_id=<?= $p->stakeholder_id ?>"
                                        class="bg-blue-500 py-2 px-4 text-white rounded-md">Editar</a>
                                    <a href="<?= BASE_URL ?>/app/actions/remove_supplier.php?supplier_id=<?= $p->stakeholder_id ?>"
                                        class="bg-red-500 py-2 px-4 text-white rounded-md">Excluir</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
