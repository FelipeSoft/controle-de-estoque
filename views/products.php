...
<main class="w-screen">
    ...
    <section class="mx-auto container my-10">
        ...
        <div>
            ...
            <form action="<?= $id ? "../app/actions/edit_product.php" : "../app/actions/new_product.php" ?>"
                method="POST" class="mb-16">
                <div class="grid grid-cols-3 gap-6">
                    <input type="text" value="<?= $id ?>" name="id" hidden>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Custo (R$)
                        <input name="product_cost" value="<?= $cost ?? ""; ?>" type="text" placeholder="ex: 12,78"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Preço Unitário (R$)
                        <input name="product_price" value="<?= $price ?? ""; ?>" type="text" placeholder="ex: 15,78"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                    <label class="flex flex-col mb-4 text-blue-500 font-regular">
                        Estoque Mínimo
                        <input name="product_min_stock" value="<?= $min_stock ?? ""; ?>" type="text"
                            placeholder="ex: 20"
                            class="w-full outline-0 focus:border-blue-500 border-2 border-gray-300 py-2 px-4 rounded-md">
                    </label>
                </div>
                    ...
                    <tbody>
                        <?php foreach ($products as $product): ?>
                            <tr class="border-2 even:bg-gray-200">
                                ...
                                <td class="py-2 px-4 text-xs">
                                    <?= "R$ " . number_format($product["product"]->cost, 2, ",", "."); ?>
                                </td>
                                <td class="py-2 px-4 text-xs">
                                    <?= "R$ " . number_format($product["product"]->cost * 1.5, 2, ",", "."); ?>
                                </td>
                                <td class="py-2 px-4 text-xs">
                                ...
                                <td class="py-2 px-4 text-xs">
                                    <?= $product["product"]->min_stock; ?>
                                </td>
                                <td class="py-2 px-4 text-xs">
                                    <?= abs($product["current_stock"]); ?>
                                </td>
                                ...
                                <td class="p-4 flex items-center justify-center gap-2">
                                    <a href="<?= BASE_URL ?>/views/products.php?product_id=<?= $product["product"]->product_id ?>"
                                        class="bg-blue-500 py-2 px-4 text-white rounded-md">Editar</a>
                                    <a href="<?= BASE_URL ?>/app/actions/remove_product.php?product_id=<?= $product["product"]->product_id ?>"
